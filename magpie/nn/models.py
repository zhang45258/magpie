...
#optimizer是我加的，用于选择模型，调参，'Adam'是默认
def get_nn_model(nn_model, embedding, output_length, optimizer):
    if nn_model == 'cnn':
        return cnn(embedding_size=embedding, output_length=output_length, optimizer=optimizer)
    elif nn_model == 'rnn':
...
def cnn(embedding_size, output_length,optimizer):
